...
export async function seedCharacterFaceAssets({
  ...
  try {
    ...
    for (const name of new Set(
      manifest.sheets.flatMap((sheet) => sheet.boundOriginalNames),
    )) {
      ...
      const ids = new Set(
        (exact.length
          ? exact
          : aliases.results.filter((row) => key(row.name) === key(name))
        ).map((row) => row.id),
      );
    ...
    const checkedSheets = manifest.sheets.map((sheet) => {
      if (!/^assets\/[a-f0-9]{64}\.(png|gif|jpg|bmp)$/.test(sheet.file))
        throw new Error(`脸图路径无效：${sheet.file}`);
      const bytes = readFileSync(resolve(manifestDirectory, sheet.file));
      const dimensions = imageSize(bytes);
      if (
        createHash("sha256").update(bytes).digest("hex") !== sheet.sha256 ||
        bytes.length !== sheet.sizeBytes ||
        dimensions.width !== sheet.width ||
        dimensions.height !== sheet.height
      )
        throw new Error(`脸图校验失败：${sheet.file}`);
      return {
        ...sheet,
        contentType: {
          png: "image/png",
    ...
    await Promise.all(
      Array.from(
        { length: Math.max(1, Math.min(64, concurrency)) },
        async () => {
    ...
    for (let start = 0; start < checkedSheets.length; start += 100) {
      await database.batch(
        checkedSheets.slice(start, start + 100).flatMap((sheet) => {
          const source = sheet.sources[0];
          return [
            database
              .prepare(
                "INSERT INTO blobs(sha256,size_bytes,content_type_hint,observed_ext,verified_at) VALUES (?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(sha256) DO UPDATE SET content_type_hint=excluded.content_type_hint",
              )
              .bind(
                sheet.sha256,
                sheet.sizeBytes,
                sheet.contentType,
                sheet.file.split(".").at(-1),
              ),
            database
              .prepare(
                "INSERT OR IGNORE INTO face_sheets(blob_sha256,width_px,height_px,source_kind,source_page_url,source_image_url,source_page_title,source_section_title,source_order,library_status) VALUES (?,?,?,?,?,?,?,?,?,'approved')",
              )
              .bind(
                sheet.sha256,
                sheet.width,
            ...
            ...sheet.boundOriginalNames.map((name) =>
              database
                .prepare(
                  "INSERT OR IGNORE INTO character_face_sheet_bindings(character_id,face_sheet_id,sort_order) SELECT ?,id,? FROM face_sheets WHERE blob_sha256=? AND library_status='approved'",
                )
    ...
    for (const portrait of manifest.defaults) {
      ...
      if (!characterId)
        throw new Error(`默认头像角色不存在：${portrait.originalName}`);
      await database.batch([
        database
          .prepare(
            `INSERT OR IGNORE INTO character_portrait_refs(character_id,face_sheet_id,cell_row,cell_column)
    ...
    console.log(
      `脸图及角色绑定已登记：${checkedSheets.length} 张；已有默认头像保留。`,
    );
  } finally {
