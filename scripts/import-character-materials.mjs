...
export async function seedCharacterMaterials({
  dryRun = false,
  manifestPath = "data/character-materials/manifest.json",
  faceManifestPath = "data/character-face-sheets/manifest.json",
} = {}) {
  manifestPath = resolve(manifestPath);
  const manifest = JSON.parse(readFileSync(manifestPath, "utf8"));
  if (
    manifest.schema !== "viprpg-character-material-library.v1" ||
    !Array.isArray(manifest.materials)
  ) {
    throw new Error("角色素材清单格式不受支持");
  }
  if (!dryRun)
    await seedCharacterFaceAssets({ manifestPath: faceManifestPath });
  ...
  try {
    ...
    const [characters, aliases, blobs, characterSources] = await db.batch([
      ...
      db.prepare(
        "SELECT character_id AS id,name FROM character_aliases WHERE language='ja'",
      ),
      db.prepare("SELECT sha256,size_bytes,status FROM blobs"),
      db.prepare(
        "SELECT character_id AS characterId,url FROM character_sources ORDER BY character_id,sort_order,url",
      ),
    ]);
    const pagesByCharacter = new Map();
    for (const source of characterSources.results) {
      if (!pagesByCharacter.has(source.characterId))
        pagesByCharacter.set(source.characterId, []);
      pagesByCharacter.get(source.characterId).push(source.url);
    }
    const identities = new Map();
    for (const name of new Set(
      manifest.materials.flatMap((material) => material.boundOriginalNames),
    )) {
      if (typeof name !== "string") throw new Error("素材角色名称格式不合法");
      const exact = characters.results.filter(
        (row) => nameKey(row.name) === nameKey(name),
      );
      const ids = new Set(
        (exact.length
          ? exact
          : aliases.results.filter((row) => nameKey(row.name) === nameKey(name))
        ).map((row) => row.id),
      );
    ...
    const existingBlobs = new Map(
      blobs.results.map((blob) => [blob.sha256, blob]),
    );
    const seen = new Set();
    const records = manifest.materials.map((material) => {
      if (
        !/^[a-f0-9]{64}$/.test(material.sha256) ||
        !["charset", "monster", "other"].includes(material.kind)
      ) {
      ...
      seen.add(key);
      if (
        !Array.isArray(material.boundOriginalNames) ||
        !material.boundOriginalNames.length
      )
        throw new Error(`素材没有角色归属：${key}`);
      const fileMatch = /^assets\/([a-f0-9]{64})\.(png|gif|jpg|bmp)$/.exec(
        material.file,
      );
      if (!fileMatch || fileMatch[1] !== material.sha256)
        throw new Error(`素材文件路径不合法：${key}`);
      const filePath = resolve(dirname(manifestPath), material.file);
      const bytes = readFileSync(filePath);
      if (
        bytes.length !== material.sizeBytes ||
      ...
      if (
        existing &&
        (existing.status !== "active" || existing.size_bytes !== bytes.length)
      ) {
      ...
      return {
        ...
        characterIds: [
          ...new Set(
            material.boundOriginalNames.map((name) => identities.get(name)),
          ),
        ],
      };
    });
    const counts = Object.fromEntries(
      ["charset", "monster", "other"].map((kind) => [
        kind,
    ...
    const compareSources = (a, b) =>
      ...
      a.pageUrl.localeCompare(b.pageUrl, "en", { numeric: true }) ||
    ...
    for (const record of records) {
      ...
      for (const characterId of record.characterIds) {
        ...
        const sources = record.sources
          .filter((source) =>
            source.originalNames.some(
              (name) => identities.get(name) === characterId,
            ),
          )
          .map((source) => {
            if (
              ...
              throw new Error(
                `素材缺少源站顺序：${record.kind}:${record.sha256}`,
              );
          ...
          .sort(compareSources);
        if (!sources.length)
          throw new Error(`素材缺少角色来源：${record.kind}:${record.sha256}`);
        if (!bindingsByCharacter.has(characterId))
          bindingsByCharacter.set(characterId, []);
        bindingsByCharacter
          .get(characterId)
          .push({ record, source: sources[0] });
      }
    }
    for (const [characterId, bindings] of bindingsByCharacter) {
      bindings.sort(
        (a, b) =>
          compareSources(a.source, b.source) ||
          a.record.sha256.localeCompare(b.record.sha256),
      );
      bindings.forEach(({ record }, order) =>
        record.sortOrders.set(characterId, order),
      );
    }
    const summary = {
      materials: records.length,
      bindings: records.reduce(
        (sum, record) => sum + record.characterIds.length,
        0,
      ),
      categories: counts,
    };
    ...
    const uniqueFiles = [
      ...new Map(records.map((record) => [record.sha256, record])).values(),
    ];
    const pending = uniqueFiles.filter(
      (record) => objects.get(blobKey(record.sha256)) !== record.sizeBytes,
    );
    let cursor = 0;
    let uploaded = 0;
    const timer = setInterval(
      () => console.log(`本地素材图片：补传 ${uploaded}/${pending.length}`),
      15_000,
    );
    try {
        ...
        Array.from({ length: 16 }, async () => {
          while (cursor < pending.length) {
            ...
            if (
              ...
              throw new Error(`导入期间文件发生改变：${record.file}`);
    ...
    for (let start = 0; start < records.length; start += 100) {
      const statements = records.slice(start, start + 100).flatMap((record) => [
        db
          .prepare(
            `INSERT INTO blobs(sha256,size_bytes,content_type_hint,observed_ext,verified_at)
        ...
        ...record.characterIds.map((characterId) =>
          db
            .prepare(
              `INSERT INTO character_material_bindings(character_id,material_id,sort_order)
          ...
          ON CONFLICT(character_id,material_id) DO UPDATE SET sort_order=excluded.sort_order`,
            ...
            .bind(
              characterId,
              record.sortOrders.get(characterId),
              record.sha256,
              record.kind,
            ),
        ),
      ...
      if (start % 1000 === 0)
        console.log(
          `本地素材登记：${Math.min(start + 100, records.length)}/${records.length}`,
        );
    }
    console.log(
      `本地素材导入完成：${JSON.stringify({ ...summary, uploaded })}`,
    );
    return summary;
  } finally {
