...
export function appendGeneratedCharacterMaterials(materials, faces) {
  const seed = JSON.parse(
    readFileSync("data/character-classification-bootstrap.json", "utf8"),
  );
  const collection = resolve("output/playwright/character-materials");
  const readOrder = createSourceOrderReader(collection);
  const pages = new Map();
  const materialMap = new Map(
    materials.map((record) => [`${record.kind}:${record.sha256}`, record]),
  );
  const faceMap = new Map(
    faces.sheets.map((record) => [record.sha256, record]),
  );
  const missingPages = new Set();
  let references = 0;
  for (const assignment of seed.materialAssignments ?? []) {
    const pageId =
      /^https:\/\/w\.atwiki\.jp\/viprpg_sozai\/pages\/(\d+)\.html$/.exec(
        assignment.pageUrl,
      )?.[1];
    if (!pageId) throw new Error(`Invalid source page: ${assignment.pageUrl}`);
    const file = resolve(collection, `${pageId}.json`);
    if (!existsSync(file)) {
      missingPages.add(assignment.pageUrl);
      continue;
    }
    if (!pages.has(pageId))
      pages.set(pageId, JSON.parse(readFileSync(file, "utf8")));
    const page = pages.get(pageId);
    for (const item of page.images) {
      if (
        assignment.sectionPrefix &&
        !item.sectionPath.some((part) =>
          part.startsWith(assignment.sectionPrefix),
        )
      )
        continue;
      if (assignment.filenames && !assignment.filenames.includes(item.filename))
        continue;
      if (
        !item.file ||
        !/^assets\/[a-f0-9]{64}\.(png|gif|jpg|bmp)$/.test(item.file)
      )
        throw new Error(`Download source image first: ${item.url}`);
      ...
      if (
        ...
        throw new Error(`Invalid cached asset: ${item.url}`);
      ...
      const directory = isFace
        ? "data/character-face-sheets"
        : "data/character-materials";
      mkdirSync(resolve(directory, "assets"), { recursive: true });
      if (!existsSync(resolve(directory, item.file)))
        copyFileSync(
          resolve(collection, item.file),
      ...
      record.boundOriginalNames = [
        ...new Set([...record.boundOriginalNames, ...assignment.originalNames]),
      ];
      const source = isFace
        ? {
            pageUrl: page.url,
            ...
            imageUrl: item.url,
            sectionTitle: item.sectionPath.join(" / "),
        ...
        : {
            ...
            sourceOrder,
            originalNames: assignment.originalNames,
            decision: "source_section",
          };
      const existingSource = record.sources.find(
        (row) => row.pageUrl === page.url && row.imageUrl === item.url,
      );
      if (!existingSource) record.sources.push(source);
      else if (!isFace)
        existingSource.originalNames = [
          ...new Set([
            ...existingSource.originalNames,
            ...assignment.originalNames,
          ]),
        ];
      if (isFace) {
        record.bindingPriorities ??= {};
        for (const name of assignment.originalNames)
          record.bindingPriorities[name] ??= 1;
      }
      references++;
    }
  }
  if (missingPages.size)
    throw new Error(
      `Collect source pages first: ${[...missingPages].join(", ")}`,
    );
  const configuredPortraits = new Set(
    faces.defaults.map((portrait) => portrait.originalName),
  );
  const missingDefaultPortraits = [
    ...new Set(faces.sheets.flatMap((sheet) => sheet.boundOriginalNames)),
  ].filter((name) => !configuredPortraits.has(name));
  if (missingDefaultPortraits.length) {
    console.warn(
      `脸图已关联但尚未选择默认格子（${missingDefaultPortraits.length}）：${missingDefaultPortraits.join("、")}。请在脸图清单 defaults 中明确选择；多人共用脸图不能统一取首格。`,
    );
  }
