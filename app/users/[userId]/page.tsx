...
export default async function PublicUserPage({ params }: { params: Promise<{ userId: string }> }) {
  ...
  const [played, favorites, catalogs, comments, discussions] = await Promise.all([
    visibility.history ? searchUserWorks({ userId: user.id, kind: "played", pageSize: 4 }) : null,
    visibility.favorites ? searchUserWorks({ userId: user.id, kind: "favorite", pageSize: 4 }) : null,
    visibility.catalogs ? searchCatalogsForOwner({ userId: user.id, pageSize: 3 }) : null,
    visibility.comments ? searchUserComments({ userId: user.id, publicOnly: true, pageSize: 3 }) : null,
    visibility.discussions ? publicUserDiscussions(getForumRuntime(), user.id, { pageSize: 3 }) : null,
  ]);
  const hasVisibleSections = visibility.history || visibility.favorites || visibility.catalogs || visibility.comments || visibility.discussions;
  return (
