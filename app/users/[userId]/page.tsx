...
export default async function PublicUserPage({ params }: { params: Promise<{ userId: string }> }) {
  ...
  const [played, favorites, catalogs, comments] = await Promise.all([
  ...
  const hasVisibleSections = visibility.history || visibility.favorites || visibility.catalogs || visibility.comments;
  return (
