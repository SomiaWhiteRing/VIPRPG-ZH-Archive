...
export function VirtualList({
  ...
  const positions = useMemo(() => {
    ...
    for (let index = 0; index < itemKeys.length; index++) {
      ...
      const height =
        heights[itemKeys[index]] ??
        (sample ? sample.total / sample.count : estimateHeight(index));
  ...
  const anchorPositions = useMemo(
    () =>
      new Map(
        itemKeys.flatMap((key, index) =>
          (anchorKeys?.[index] ?? [key]).map(
            (anchor) => [anchor, index] as const,
          ),
        ),
      ),
    [itemKeys, anchorKeys],
  );
  const stickyByIndex = useMemo(
    () => new Map(stickyItems?.map((item) => [item.index, item])),
    [stickyItems],
  );
  const stickyOffset = useCallback(
    (index: number) => {
      const ancestors =
        stickyByIndex.get(index)?.ancestors ??
        stickyItems
          ?.filter((item) => item.index < index && item.endIndex > index)
          .map((item) => item.index) ??
        [];
      return ancestors.reduce(
        (height, ancestor) =>
          height + positions[ancestor + 1] - positions[ancestor],
        0,
      );
    },
  ...
  const recordViewport = useCallback(() => {
    ...
    if (!navigationRef.current) {
      ...
      const anchoredIndex = anchor
        ? anchorPositions.get(anchor.key)
        : undefined;
      // A programmatic correction must retain the exact member, including navigation below sticky headings.
      if (
        !anchor ||
        anchoredIndex === undefined ||
        Math.abs(top - positions[anchoredIndex] - anchor.offset) > 1
      ) {
        const key = anchorKeys?.[index]?.[0] ?? itemKeys[index];
        anchorRef.current =
          top >= 0 && top < positions[itemKeys.length] && key
            ? { key, offset: top - positions[index] }
            : null;
      }
    }
    setViewport((current) =>
  ...
  useLayoutEffect(() => {
    recordViewportRef.current = recordViewport;
  }, [recordViewport]);
  ...
  useLayoutEffect(() => {
    navigationStepRef.current = (time) => {
      ...
      const index = navigation
        ? anchorPositions.get(navigation.key)
        : undefined;
      ...
      const elapsed =
        navigation.duration === 0
          ...
          : Math.max(
              0,
              Math.min(1, (time - navigation.startedAt) / navigation.duration),
            );
      const progress =
      ...
      const target = Math.max(
        0,
        Math.min(
          start + positions[index] + offset - scrollOffset,
      ...
      const fraction =
        elapsed === 1
          ? 1
          : (progress - navigation.progress) / (1 - navigation.progress);
      window.scrollTo({
        top: window.scrollY + (target - window.scrollY) * fraction,
        behavior: "instant",
      });
      ...
      if (elapsed === 1) {
        ...
        navigationFrameRef.current = requestAnimationFrame((nextTime) =>
          navigationStepRef.current(nextTime),
        );
      }
    };
  }, [
  ...
  const measure = useCallback((key: string, height: number) => {
    if (height <= 0) return;
    setHeights((current) =>
      Math.abs((current[key] ?? 0) - height) < 0.5
        ? current
        : { ...current, [key]: height },
    );
  }, []);
  ...
  const end = Math.min(
    itemKeys.length,
    findItem(positions, viewport.top + viewport.height) + OVERSCAN + 1,
  );
  ...
  return (
    <div
      aria-label={label}
      className={cn(
        "[overflow-anchor:none] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary",
        stickyItems && "relative isolate",
        className,
      )}
      ref={viewportRef}
      role="region"
    ...
    >
      ...
      {itemKeys.slice(first, end).map((key, offset) =>
        stickyByIndex.has(first + offset) ? (
          <div
            aria-hidden
            key={key}
            style={{
              height: positions[first + offset + 1] - positions[first + offset],
            }}
          />
        ) : (
          <MeasuredItem
      ...
      <div
        aria-hidden
        style={{ height: positions[itemKeys.length] - positions[end] }}
      />
      ...
      {stickyItems?.map((item) => (
        <div
          ...
          style={{
            top: positions[item.index],
            height: positions[item.endIndex] - positions[item.index],
            zIndex: 20 - item.ancestors.length,
          }}
        >
          <div
            className="pointer-events-auto sticky"
            style={{ top: scrollOffset + stickyOffset(item.index) }}
          >
            <MeasuredItem
              itemKey={itemKeys[item.index]}
              onMeasure={measure}
              measurementWidth={measurementWidth}
            >
...
function MeasuredItem({
  ...
  return (
    <div className="flow-root" ref={ref}>
      {children}
    </div>
  );
}
