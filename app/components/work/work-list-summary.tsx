...
export function WorkListSummary({
  href,
  title,
  originalTitle,
  releaseDate,
  ...
  return (
    <div className="min-w-0 flex-1">
      ...
      {originalTitle ? (
        <p
          className={cn(
            "mt-0.5 text-[12.5px] text-muted",
            truncateOriginal && "truncate",
          )}
        >
          {originalTitle}
        </p>
      ...
      <p className="mt-1 font-mono text-xs text-muted">
        {[releaseDate, engineLabel(engineFamily), languageLabel(language)]
          .filter(Boolean)
          .join(" / ")}
      </p>
      {children}
