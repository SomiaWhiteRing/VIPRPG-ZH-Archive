        ...
        >
          {inAdmin ? (
            ...
            <>
              ...
              <NavLink href="/games" label="游戏库" pathname={pathname} />
      ...
      {!inAdmin && mobileMenuOpen ? (
        <nav className="fixed inset-x-0 top-14 z-50 border-b border-border bg-background shadow-lg md:hidden" aria-label="移动端导航">
          <div className="mx-auto w-[min(1280px,calc(100vw-2rem))] py-2">
            ...
            <Link
              aria-current={pathname === "/games" || pathname.startsWith("/games/") ? "page" : undefined}
              className={`block rounded-md px-3 py-2.5 text-sm font-semibold ${pathname === "/games" || pathname.startsWith("/games/") ? "bg-primary text-primary-foreground" : "hover:bg-muted/15"}`}
              href="/games"
              onClick={() => setMobileMenuOpen(false)}
            >
              游戏库
