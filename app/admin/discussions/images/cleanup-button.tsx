...
export function ImageCleanupButton({ id, author, topicId, postNumber, retry }: {
  id: string;
  author: string;
  topicId: number | null;
  ...
  return (
    <AlertDialog open={open} onOpenChange={(value) => { if (!busy) setOpen(value); }}>
      ...
      <AlertDialogContent className="rounded-md">
        <AlertDialogTitle className="text-lg font-bold">永久删除图片？</AlertDialogTitle>
        <AlertDialogDescription className="break-words text-sm text-muted">
          {author} · {topicId ? `主题 ${topicId} · #${postNumber}` : "未关联帖子"}
