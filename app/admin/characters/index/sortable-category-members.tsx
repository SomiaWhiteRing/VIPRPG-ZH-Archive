...
export function SortableCategoryMembers({
  nodes,
  busy,
  sortDisabled,
  onSelect,
  ...
  function rememberPositions() {
    positions.current = new Map(
      [...elements.current].map(([id, element]) => [
        id,
  ...
  function previewMove(
    ...
    if ((from < target && !afterMidpoint) || (from > target && afterMidpoint))
      return;
    ...
    const next: DragPreview = {
      ...
      move: {
        ...
        targetCharacterId: node.character.id,
        position: from < target ? "after" : "before",
      },
    };
    rememberPositions();
    drag.current = next;
    ...
    <div
      ...
      onDrop={(event) => {
        ...
        if (
          !sortDisabled &&
          current.move &&
          current.nodes.some((node, index) => node.id !== nodes[index]?.id)
        )
          onReorder(current.nodes, current.move);
      }}
    >
      {visibleNodes.map((node) => {
        ...
        return (
          <div
            ...
            draggable={movable}
            onDragStart={(event) => {
              if (!movable || node.kind !== "character") {
                event.preventDefault();
                return;
              }
            ...
            onDragOver={(event) => previewMove(event, node)}
            onDragEnd={resetDrag}
          >
            <Button
              type="button"
              ...
              className={cn(
                ...
                moving &&
                  "border-dashed border-primary bg-primary/10 opacity-40",
              )}
              title={movable ? `拖动排序：${node.label}` : undefined}
              onClick={() => {
                if (!drag.current) onSelect(node);
              }}
            >
              {node.kind === "character" ? (
