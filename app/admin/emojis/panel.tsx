...
export function EmojiAdminPanel() {
  ...
  useEffect(() => {
    ...
    void fetch("/api/admin/emojis", { credentials: "same-origin" })
      .then(async (response) => ({
        response,
        body: (await response.json()) as { emojis?: CustomEmojiDto[] },
      }))
      .then(({ response, body }) => {
  ...
  async function toggle(emoji: CustomEmojiDto) {
    const response = await fetch("/api/admin/emojis", {
      ...
      body: JSON.stringify({
        id: emoji.id,
        status: emoji.status === "active" ? "retired" : "active",
      }),
    });
  ...
  return (
    <div className="grid gap-5">
      <section className="grid gap-3 rounded-md border border-border bg-muted/10 p-4">
        ...
        <div>
          <Button
            disabled={busy || !file || !shortcode.trim() || !name.trim()}
            onClick={upload}
            type="button"
          >
            上传
        ...
        {emojis.map((emoji) => (
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              :{emoji.shortcode}: · {emoji.name} · {emoji.category}
            </span>
            <Button
