...
const roles = [{ value: "author", label: "作者" }, { value: "translator", label: "译者" }, ...EXTRA_STAFF_ROLES] satisfies { value: StaffCredit["roleKey"]; label: string }[];
...
export function WorkStaffFields({ credits, suggestions }: { credits: StaffCredit[]; suggestions: CreatorSuggestion[] }) {
  ...
  return <fieldset className="grid min-w-0 gap-3">
    ...
    {rows.map((row, index) => <StaffCreditRow key={row.id} id={`admin-${row.id}`} index={index} value={row} roles={roles} suggestions={suggestions} showNotes requireRoleLabel
      onChange={(patch) => update(row.id, { ...patch, roleKey: patch.roleKey || row.roleKey })}
