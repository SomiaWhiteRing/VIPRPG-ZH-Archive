...
export function WorkStaffFields({ credits, suggestions }: { credits: StaffCredit[]; suggestions: CreatorSuggestion[] }) {
  ...
  return <fieldset className="grid min-w-0 gap-3">
    ...
    {rows.map((row, index) => <div className={`grid min-w-0 items-start gap-2 border-b border-border pb-3 ${row.roleKey === "other" ? "md:grid-cols-[7rem_7rem_minmax(0,1fr)_minmax(8rem,1fr)_auto]" : "md:grid-cols-[7rem_minmax(0,1fr)_minmax(8rem,1fr)_auto]"}`} key={row.id}>
      ...
      {row.roleKey === "other" ? <Input aria-label={`第 ${index + 1} 条署名的职务名称`} placeholder="职务名称" required value={row.roleLabel ?? ""} onChange={(event) => update(row.id, { roleLabel: event.target.value })} /> : null}
