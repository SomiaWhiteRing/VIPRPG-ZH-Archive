...
export async function runStorageConsistencyCheck(
  ...
  const [blobCheck, corePackCheck, manifestCheck, r2Scan] = await Promise.all([
    checkD1ObjectsInR2(runtime, "blob", blobRows),
    checkD1ObjectsInR2(runtime, "core_pack", corePackRows),
    checkD1ObjectsInR2(runtime, "manifest", manifestRows),
    scanR2Objects(runtime, r2ScanLimit),
  ]);
...
async function scanR2Objects(
  ...
  const listedObjects = await listR2Objects(runtime, limit);
  const known = await findKnownR2Sha256(runtime, listedObjects.objects);
...
async function findKnownR2Sha256(
  runtime: AppRuntime,
  objects: R2ListedObject[],
): Promise<{
  blob: Set<string>;
...
async function listArchiveVersionPurgeCandidates(
  ...
  const rows = await archiveVersionPurgeCandidatesStatement(
    getD1(runtime),
    graceDays,
    limit,
...
async function sweepGcRows(
  ...
  for (const row of rows) {
    ...
    const reserved = await markGcCandidatePurging(
      runtime,
      type,
      row.id,
      graceDays,
    ...
    try {
      ...
      await markGcCandidatePurged(runtime, type, row.id);
      purged.push(object);
    } catch (error) {
      await restoreGcCandidateActive(runtime, type, row.id);
      failed.push({
        ...object,
        error: error instanceof Error ? error.message : "Unknown error",
      });
...
async function markGcCandidatePurged(
  ...
  const result = await getD1(runtime).prepare(sql).bind(id).run();
...
async function restoreGcCandidateActive(
  ...
  await getD1(runtime).prepare(sql).bind(id).run();
}
