...
import {
  ...
  replyNotificationStatement,
} from "./notifications";
import type { ForumRuntime } from "./runtime";
import {
  contentIndexStatements,
  searchVisibilityStatement,
  topicSearchDocuments,
} from "./search-index";
...
function guard(
  ...
  return ctx.db
    ...
    .bind(token, token, topic.id, topic.revision, actor.id, ...predicate.args);
}
...
export async function publishForum(
  ...
  if (kind === "topic") {
    ...
    const results = await db.batch([
      ...
      db
        .prepare(
          "UPDATE forum_topics SET last_activity_at=created_at WHERE user_id=? AND request_key=? AND write_token=?",
        )
        .bind(actor.id, identity.key, token),
      ...tagStatements(
        ctx,
  ...
  const parent = await contentIdentity(
    ctx,
    kind === "post"
  ...
  if (kind === "comment" && targetId) {
    const target = await contentIdentity(ctx, {
      kind: "comment",
      id: targetId,
    });
  ...
  if (kind === "comment")
    statements.push(
      db
        .prepare(
          `UPDATE forum_posts SET next_comment_number=next_comment_number+1 WHERE id=? AND ${tokenGate.sql}`,
        )
        .bind(parent.id, ...tokenGate.args),
      db
        .prepare(
          `UPDATE forum_topics SET reply_count=reply_count+1,last_activity_at=CURRENT_TIMESTAMP,last_comment_id=(SELECT id FROM forum_post_comments WHERE user_id=? AND revision=?),last_post_id=NULL WHERE id=? AND write_token=?`,
        )
        .bind(actor.id, token, topic.id, token),
    );
  statements.push(
...
export async function editForum(
  ...
  const statements = [
    ...
    ctx.db
      ...
      .bind(body, token, row.id, ...gate.args),
  ];
  if (title)
    statements.push(
      ctx.db
        .prepare("UPDATE forum_topics SET title=? WHERE id=? AND write_token=?")
        .bind(title, topic.id, token),
    );
  if (tags)
    statements.push(...tagStatements(ctx, topic.id, tags, actor, token));
  if (target.kind !== "comment")
    statements.push(...imageStatements(ctx, images, actor.id, token, offsets));
  statements.push(
    ...contentIndexStatements(
      ctx,
      target.kind === "comment" ? "comment" : "post",
      actor.id,
      token,
      title ?? "",
      body,
...
export async function deleteForum(
  ...
  if (
    ...
    (await ctx.db
      .prepare(
        "SELECT 1 FROM forum_posts WHERE topic_id=? AND post_number<>1 AND status<>'deleted' UNION ALL SELECT 1 FROM forum_post_comments c JOIN forum_posts p ON p.id=c.post_id WHERE p.topic_id=? AND c.status<>'deleted' LIMIT 1",
      )
      .bind(topic.id, topic.id)
      .first())
  )
...
export async function moderateForum(
  ...
  const restoreImages =
    action === "restore" && target.kind !== "comment"
      ? {
  ...
  if (
    action === "restore" &&
    !(await db
      .prepare(`SELECT 1 AS ok WHERE ${restoreImages.sql}`)
      .bind(...restoreImages.args)
      .first())
  )
  ...
  if (action === "hide" || action === "restore") {
    ...
    statements.push(
      searchVisibilityStatement(
        db,
        topicSearchDocuments(target.kind),
