...
export function searchVisibilityStatement(
  db: D1Database,
  selection: string,
  args: (number | string)[],
) {
  return db
    .prepare(
      `UPDATE forum_search_index SET scope=(
    SELECT ${searchScopeSql} FROM forum_search_documents d WHERE d.id=forum_search_index.rowid
  ) WHERE rowid IN(${selection})`,
    )
    .bind(...args);
}

export function topicSearchDocuments(
  kind: "topic" | "post" | "comment",
): string {
  if (kind === "comment")
    return "SELECT id FROM forum_search_documents WHERE comment_id=?";
...
export function userSearchVisibilityStatement(db: D1Database, userId: number) {
  return searchVisibilityStatement(
    db,
    `SELECT d.id FROM forum_search_documents d WHERE
...
export function contentIndexStatements(
  ...
  if (operation === "insert")
    statements.push(
      ctx.db
        .prepare(`INSERT INTO forum_search_documents(${column}) ${source}`)
        .bind(...args),
    );
  else
    statements.push(
      ctx.db
        .prepare(`DELETE FROM forum_search_index WHERE rowid IN(${documents})`)
        .bind(...args),
    );
  if (operation !== "delete")
    statements.push(
