...
export async function forumSitemap(ctx: ForumRequestRuntime, shard?: number) {
  ...
  if (shard === undefined) {
    const row = await ctx.db
      .prepare("SELECT MAX(id) AS maximum FROM forum_topics")
    ...
    if (
      ...
      shard < 0 ||
      !Number.isSafeInteger((shard + 1) * 1000)
    )
      throw new HttpError(404, "站点地图不存在。");
    const rows = await ctx.db
      .prepare(
        "SELECT id,updated_at FROM forum_public_topics WHERE id>? AND id<=? ORDER BY id LIMIT 1000",
      )
      .bind(shard * 1000, (shard + 1) * 1000)
