...
export async function forumEngagement(
  ...
  if (
    postIds.length > 20 ||
    postIds.some((id) => !Number.isSafeInteger(id) || id < 1)
  )
  ...
  if (own) {
    const reply = await ctx.db
      .prepare(
        `SELECT id FROM forum_posts WHERE topic_id=? AND post_number<>1 AND status<>'deleted'
  ...
  const likes = await ctx.db
    .prepare(
      `SELECT l.post_id FROM forum_post_likes l JOIN forum_public_posts p ON p.id=l.post_id
    WHERE l.user_id=? AND p.topic_id=? AND l.post_id IN(SELECT value FROM json_each(?))`,
    )
    .bind(viewer.id, topicId, JSON.stringify(postIds))
    .all<{ post_id: number }>();
  return {
    viewer,
    liked: likes.results.map((r) => r.post_id),
    topicCapabilities: mapTopic(topic, viewer, []).capabilities,
  };
}
