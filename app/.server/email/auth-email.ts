import { getEmailFrom } from "@/app/.server/auth/config";
...
async function sendAuthEmail(
  ...
  try {
    await getCloudflareEnv(runtime).EMAIL.send({
      from: getEmailFrom(runtime),
      to: input.to,
...
function normalizeEmailSendError(error: unknown): Error {
  ...
  if (
    message
      .toLowerCase()
      .includes("destination address is not a verified address")
  ) {
    return new Error(
