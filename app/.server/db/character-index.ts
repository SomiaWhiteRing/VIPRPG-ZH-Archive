...
function structureStatements(database: D1Database) {
  return [
    database.prepare(
      "SELECT id,parent_id AS parentId,label,original_name AS originalName,source_url AS sourceUrl,sort_order AS sortOrder FROM character_categories ORDER BY sort_order,id",
    ),
    database.prepare(
      "SELECT category_id AS categoryId,character_id AS characterId,sort_order AS sortOrder,display_name AS displayName,original_name AS originalName FROM character_category_memberships ORDER BY sort_order,character_id",
    ),
  ];
}
...
export async function readCharacterIndex(
  ...
  return {
    ...
    characters: (await listPublicCharacterIndex(runtime)).map((character) => ({
      ...
      sourceUrls: urlsByCharacter.get(character.id) ?? [],
      commentCount: commentCounts.get(character.id) ?? 0,
      materialCount: materialCounts.get(character.id) ?? 0,
    })),
  };
}
export async function readCharacterCounts(
  runtime: AppRuntime,
  ...
  const selected =
    characterIds === undefined
      ? null
      : JSON.stringify([...new Set(characterIds)]);
  const commentFilter =
    selected === null
      ? ""
      : " AND c.character_id IN (SELECT value FROM json_each(?))";
  ...
  const commentCounts = new Map(
    (comments.results as { characterId: number; count: number }[]).map(
      (row) => [row.characterId, row.count],
    ),
  );
  const materialCounts = new Map(
    (materials.results as { characterId: number; count: number }[]).map(
      (row) => [row.characterId, row.count],
    ),
  );
...
function text(
  ...
  if ((required && !result) || result.length > max)
    throw new HttpError(
      400,
      `${field}${required ? "不能为空，且" : ""}不能超过 ${max} 个字符`,
    );
  return result || null;
}
export async function updateCharacterIndex(
  ...
  const [categories, memberships, characters, aliases] = await database.batch([
    ...structureStatements(database),
    database.prepare(
      "SELECT id,primary_name AS primaryName,original_name AS originalName FROM characters",
    ),
    database.prepare(
      "SELECT character_id AS characterId,name,language FROM character_aliases",
    ),
  ]);
  const names = new Map<number, CharacterAliasSuggestion[]>();
  for (const row of aliases.results as (CharacterAliasSuggestion & {
    characterId: number;
  })[]) {
  ...
  const data = {
    ...
    characters: (
      ...
      ...character,
      aliases: names.get(character.id) ?? [],
    })),
  };
  const categoryId = text(body.categoryId, "分类", false, 240);
  const category = data.categories.find((item) => item.id === categoryId);
  const requireCategory = (id: string | null) => {
    if (!id || !data.categories.some((item) => item.id === id))
      throw new HttpError(404, "分类已不存在，请刷新后重试");
    return id;
  };
  const requireCharacter = (id: unknown) => {
    if (
      !Number.isSafeInteger(id) ||
      !data.characters.some((item) => item.id === id)
    )
      throw new HttpError(404, "角色已不存在，请刷新后重试");
    return id as number;
  };
  const selectedNames = (
    characterId: number,
    input: Record<string, unknown>,
    existing?: CharacterMembership,
  ) => {
    ...
    if (
      (!characterNameOptions(character, "zh").includes(displayName) &&
        displayName !== existing?.displayName) ||
      (!characterNameOptions(character, "ja").includes(originalName) &&
        originalName !== existing?.originalName)
    )
      throw new HttpError(400, "请选择该角色已有的中文名和日文名");
    return {
      displayName: displayName === character.primaryName ? null : displayName,
      originalName:
        originalName === character.originalName ? null : originalName,
    };
  };
  const children = (parentId: string | null) =>
    sortCharacterNodes([
      ...
      ...data.memberships
        ...
        .map((item) => ({
          id: characterMembershipKey(item.categoryId, item.characterId),
  ...
  const nextOrder = (parentId: string | null) =>
    Math.max(-1, ...children(parentId).map((item) => item.sortOrder)) + 1;
  if (body.operation === "saveCategory") {
    ...
    if (
      category &&
      parentId &&
      characterNodeDescendants(data.categories, category.id).has(parentId)
    )
      throw new HttpError(400, "不能移动到自身或下级分类中");
    ...
    const source = normalizeHttpUrl(
      text(body.sourceUrl, "来源链接", false, 2048),
      "来源链接",
    );
  ...
  if (body.operation === "addCharacters") {
    ...
    if (
      ...
      body.characters.length > 100
    )
      throw new HttpError(400, "请选择 1 至 100 位角色及使用的名字");
    const selected = body.characters.map((input: unknown) => {
      if (!input || typeof input !== "object" || Array.isArray(input))
        throw new HttpError(400, "角色选择格式不正确");
      const value = input as Record<string, unknown>;
      const characterId = requireCharacter(value.characterId);
      return { characterId, ...selectedNames(characterId, value) };
    });
    if (
      new Set(selected.map((item) => item.characterId)).size !== selected.length
    )
    ...
    await database.batch(
      selected.map((item, index) =>
        database
          .prepare(
            "INSERT INTO character_category_memberships(category_id,character_id,sort_order,display_name,original_name) VALUES (?,?,?,?,?) ON CONFLICT(category_id,character_id) DO NOTHING",
          )
          .bind(
            categoryId,
            item.characterId,
    ...
    return {
      categoryId,
      characterId: selected.length === 1 ? selected[0].characterId : null,
    };
  }
  if (body.operation === "saveMembership") {
    const characterId = requireCharacter(body.characterId);
    const target = requireCategory(
      text(body.targetCategoryId, "所属分类", true, 240),
    );
    const existing = data.memberships.find(
      (member) =>
        member.categoryId === categoryId && member.characterId === characterId,
    );
    ...
    if (target !== categoryId) {
      if (
        ...
        data.memberships.some(
          (member) =>
            member.categoryId === target && member.characterId === characterId,
        )
      )
        throw new HttpError(409, "该角色已在目标分类中，可直接从当前分类移除");
      statements.push(
        database
          .prepare(
            "UPDATE character_category_memberships SET category_id=?,sort_order=?,display_name=?,original_name=? WHERE category_id=? AND character_id=?",
          )
          .bind(
            target,
            nextOrder(target),
      ...
      statements.push(
        database
          .prepare(
            "UPDATE character_category_memberships SET display_name=?,original_name=? WHERE category_id=? AND character_id=?",
          )
  ...
  if (body.operation === "saveSources") {
    const characterId = requireCharacter(body.characterId);
    if (!Array.isArray(body.sourceUrls) || body.sourceUrls.length > 100)
      throw new HttpError(400, "来源链接格式不正确，最多 100 个");
    const urls = [
      ...new Set(
        body.sourceUrls.map(
          (url) =>
            normalizeHttpUrl(text(url, "来源链接", true, 2048), "来源链接")!,
        ),
      ),
    ];
    await database.batch([
      ...
      ...urls.map((url, order) =>
        database
          .prepare(
            "INSERT INTO character_sources(character_id,url,sort_order) VALUES (?,?,?)",
          )
    ...
    await database.batch(
      siblings.map((item, order) =>
        item.characterId === null
          ? database
              .prepare(
                "UPDATE character_categories SET sort_order=? WHERE id=? AND parent_id IS ?",
              )
              .bind(order, item.categoryId, parentId)
          : database
              .prepare(
  ...
  if (body.operation === "reorderTo") {
    ...
    if (body.position !== "before" && body.position !== "after")
      throw new HttpError(400, "排序位置不正确");
    ...
    if (index < 0 || target < 0)
      throw new HttpError(409, "分类成员已变化，请刷新后重试");
