...
export async function commitArchiveImport(
  ...
  await ensureCharacterFaceSheets(runtime, faceSheetHashes, input.user.id);
  ...
  if (metadata.target.mode === "update") {
    const preserved = await getD1(runtime)
  ...
  const archiveVersionId = existingArchiveVersion
    ...
    : await insertArchiveVersion(runtime, {
        workId,
        manifest,
        metadata,
        manifestSha256,
  ...
  await putManifest(runtime, manifestSha256, manifestJson, {
    workId,
    archiveVersionId,
  });

  await writeArchiveObjectLinks(runtime, {
    archiveVersionId,
    blobHashes,
    corePacks: corePackHashes.map(
      (sha256) => objectLedger.corePacks.get(sha256)!,
    ),
  });
  await finalizeArchiveCommit(runtime, {
    importJobId: job.id,
    workId,
    archiveVersionId,
    manifest,
...
export async function verifyArchiveSourceManifest(
  ...
  if (scan && cleanup) {
    ...
    if (cleanup.excluded.some((file) => !allowed.has(file.path))) {
      throw new HttpError(400, "素材排除清单未通过服务器引用复核，请关闭素材清理后重新上传");
    }
  }
...
async function resolveTargetWork(
  ...
  if (existingJobWorkId) {
    if (!(await canEditWork(runtime, existingJobWorkId, user))) {
      throw new HttpError(403, "导入任务对应的游戏不属于当前上传者");
    }
    const identity = await getD1(runtime)
      .prepare(
...
async function finalizeArchiveCommit(
  runtime: AppRuntime,
  input: {
    importJobId: number;
    workId: number;
...
async function writeArchiveObjectLinks(
  runtime: AppRuntime,
  input: {
    archiveVersionId: number;
    blobHashes: string[];
