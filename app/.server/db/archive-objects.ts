...
import { recordImportObjectUploadStatement } from "@/app/.server/db/import-jobs";
...
export async function prepareObjectUpload(
  runtime: AppRuntime,
  input: {
    kind: "blob" | "core_pack";
    sha256: string;
...
export async function assertObjectUploadAllowed(
  runtime: AppRuntime,
  input: {
    kind: "blob" | "core_pack";
    sha256: string;
...
export async function findExistingObjects(
  runtime: AppRuntime,
  input: {
    blobSha256: string[];
  ...
  const existing: ExistingObjectSet = {
    blobs: new Set(),
    corePacks: new Set(),
  };
  ...
  results.forEach((result, index) => {
    const target =
      queries[index].kind === "blob" ? existing.blobs : existing.corePacks;
...
export async function insertBlobRecords(
  runtime: AppRuntime,
  inputs: BlobRecordInput[],
): Promise<void> {
  if (inputs.length === 0) return;
  const database = getD1(runtime);
  const results = await database.batch(
    inputs.map((input) => blobRecordStatement(database, input)),
  );
...
export async function recordUploadedBlob(
  ...
  const results = await database.batch([
    ...
    recordImportObjectUploadStatement(
      database,
      {
...
export async function recordUploadedCorePack(
  runtime: AppRuntime,
  input: CorePackRecordInput & {
    importJobId: number;
    durationMs: number;
  },
): Promise<void> {
...
export async function findObjectStatuses(
  ...
  const statements = chunkArray(uniqueHashes, 100).map((chunk) =>
    database
      .prepare(
        `SELECT sha256,status FROM ${table} WHERE sha256 IN (${chunk.map(() => "?").join(",")})`,
      )
      .bind(...chunk),
  );
  const results = await database.batch<{ sha256: string; status: string }>(
    statements,
  );
  const statuses = new Map<string, string>();
  for (const result of results) {
    for (const row of result.results ?? [])
      statuses.set(row.sha256, row.status);
  }
  return statuses;
}

function blobRecordStatement(
  database: D1Database,
  input: BlobRecordInput,
): D1PreparedStatement {
  return database
...
function corePackRecordStatement(
  database: D1Database,
  input: CorePackRecordInput,
): D1PreparedStatement {
  return database
...
function assertObjectRecordChanged(
  result: D1Result,
  kind: "blob" | "core_pack",
): void {
