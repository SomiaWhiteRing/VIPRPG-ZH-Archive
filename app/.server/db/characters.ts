...
export function parseCharacterCreditSelection(
  ...
  if (!isCharacterRoleKey(value.roleKey))
    throw new HttpError(400, "角色身份不合法");
  return {
...
export async function prepareWorkCharacterStatements(input: {
  ...
  const existingCredits = credits.filter(
    (
      credit,
    ): credit is WorkCharacterCreditInput & {
  ...
  const existingRows = await resolveExistingCharacters(
    input.database,
    existingCredits,
  );
  const resolvedNewRows = await resolveNewCharacters(
    input.database,
    newCredits,
  );
  ...
  existingCredits.forEach((credit, index) => {
    ...
    if (
      input.requirePortrait &&
      !credit.portrait &&
      character.has_default_portrait !== 1
    ) {
      throw new HttpError(
        400,
  ...
  newCredits.forEach((credit, index) => {
    ...
    if (
      input.requirePortrait &&
      !credit.portrait &&
      existing?.has_default_portrait !== 1
    ) {
...
function parseFaceSheetHashes(value: unknown): string[] {
  if (!Array.isArray(value))
    throw new HttpError(400, "角色素材表列表格式不合法");
  try {
