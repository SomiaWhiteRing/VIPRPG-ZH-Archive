...
export async function getPublicCreatorDetail(
  ...
  const [creatorResult, aliasesResult] = await database.batch([
    ...
    database
      .prepare(
        `SELECT name FROM creator_aliases WHERE creator_id=? ORDER BY name`,
      )
      .bind(id),
  ]);
...
export async function searchCreatorsForAdmin(
  runtime: AppRuntime,
  input: {
    query?: string;
  ...
  const [rowsResult, countResult] = await database.batch([
    database
      .prepare(
        `${summarySql()} FROM creators c ${where} ORDER BY ${order} LIMIT ? OFFSET ?`,
      )
      .bind(...binds, pageSize, (page - 1) * pageSize),
    database
      .prepare(`SELECT COUNT(*) AS count FROM creators c ${where}`)
      .bind(...binds),
  ]);
  return {
...
export async function getCreatorForAdminEdit(
  ...
  return {
    ...
    adminWorkCredits: await listCredits(runtime, id, true),
  };
}
export async function updateCreatorForAdmin(
  runtime: AppRuntime,
  input: {
    creatorId: number;
    name: string;
  ...
  const existing = await getCreatorForAdminEdit(runtime, input.creatorId);
  ...
  const updated = await getCreatorForAdminEdit(runtime, input.creatorId);
...
export function parseCreatorEditForm(
  form: FormData,
): Parameters<typeof updateCreatorForAdmin>[1] {
