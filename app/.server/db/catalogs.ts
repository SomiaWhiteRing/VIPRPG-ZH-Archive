...
export async function searchCatalogsForOwner(
  runtime: AppRuntime,
  input: {
    userId: number;
    page?: number;
...
export async function getCatalogById(
  ...
  return loadCatalogDetail(runtime, id);
}
...
export async function updateCatalog(
  ...
  const row = await ownedCatalog(runtime, id, actor, "catalog.update_own");
...
export async function assertCatalogUpdateAllowed(
  ...
  await ownedCatalog(runtime, id, actor, "catalog.update_own");
}
...
export async function updateCatalogItem(
  ...
  await ownedCatalog(runtime, catalogId, actor, "catalog.reorder_own");
  if (
  ...
  return requiredCatalog(runtime, catalogId);
}
...
export async function addCatalogItem(
  ...
  await getD1(runtime).batch([
    getD1(runtime)
      .prepare(
        `INSERT OR IGNORE INTO catalog_items(catalog_id,work_id,note) VALUES(?,?,?)`,
      )
...
async function loadCatalogDetail(
  ...
  const row = await getD1(runtime)
    ...
    .first<Row>();
  if (!row) return null;
  const items = await getD1(runtime)
    .prepare(
      `SELECT
         ci.work_id,
         w.original_title,
...
async function requiredCatalog(
  ...
  const catalog = await getCatalogById(runtime, id);
...
async function requiredCatalogCover(
  runtime: AppRuntime,
  value: string,
): Promise<string> {
  ...
  const blob = await getD1(runtime)
    .prepare(
      `SELECT sha256
       FROM blobs
