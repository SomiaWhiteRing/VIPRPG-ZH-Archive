...
export function mapCharacterPortrait(
  row: CharacterPortraitRow,
...
export async function getCharacterPortraitConfigurationForAdmin(
  ...
  const defaultRow = (defaultResult.results?.[0] ??
    null) as CharacterPortraitRow | null;
  return {
    boundSheetIds: (bindingsResult.results ?? []).map((row) =>
      Number((row as { face_sheet_id: number }).face_sheet_id),
    ),
...
export async function getCharacterPortraitLibraryForAdmin(
  ...
  const [
    ...
    database
      .prepare(
        `SELECT m.id FROM character_material_bindings b JOIN character_materials m ON m.id=b.material_id WHERE b.character_id=? ORDER BY b.sort_order IS NULL,b.sort_order,m.id`,
      )
      .bind(characterId),
    database
      .prepare(
  ...
  return {
    sheets: (sheetsResult.results ?? []).map((row) =>
      mapAdminFaceSheet(row as AdminFaceSheetRow),
    ),
    boundSheetIds: (bindingsResult.results ?? []).map((row) =>
      Number((row as { face_sheet_id: number }).face_sheet_id),
    ),
    ...
    boundMaterialIds: (materialBindingsResult.results ?? []).map((row) =>
      Number((row as { id: number }).id),
    ),
  };
}

export async function searchCharacterMaterialsForAdmin(
  runtime: AppRuntime,
  input: {
    kind: string;
    query: string;
  ...
  if (faces) {
    const result = await database
      .prepare(
        `SELECT fs.id,fs.blob_sha256,fs.width_px,fs.height_px,fs.source_page_url,fs.source_image_url,
      ...
      .bind(...binds, MATERIAL_PAGE_SIZE + 1, input.offset)
      .all<AdminFaceSheetRow>();
    return {
      sheets: result.results
        .slice(0, MATERIAL_PAGE_SIZE)
        .map(mapAdminFaceSheet),
      materials: [],
      nextOffset:
        result.results.length > MATERIAL_PAGE_SIZE
          ? input.offset + MATERIAL_PAGE_SIZE
          : null,
    };
  }
  ...
  const result = await database
    ...
    .bind(...binds, MATERIAL_PAGE_SIZE + 1, input.offset)
    .all<AdminCharacterMaterial>();
  return {
    sheets: [],
    materials: result.results.slice(0, MATERIAL_PAGE_SIZE),
    nextOffset:
      result.results.length > MATERIAL_PAGE_SIZE
        ? input.offset + MATERIAL_PAGE_SIZE
        : null,
...
export async function registerAdminFaceSheetForCharacter(
  runtime: AppRuntime,
  input: {
    characterId: number;
    sha256: string;
  ...
  if (!row)
    throw new HttpError(500, "脸图素材表已上传，但未能绑定到当前角色。");
  return mapAdminFaceSheet(row);
}

export async function updateCharacterPortraitLibraryForAdmin(
  runtime: AppRuntime,
  input: {
    characterId: number;
    faceSheetIds: number[];
...
export async function prepareCharacterPortraitLibraryUpdate(
  runtime: AppRuntime,
  input: {
    characterId: number;
    faceSheetIds: number[];
  ...
  const selectedByHash = new Map(
    (rows.results ?? []).map((row) => [row.blob_sha256, row]),
  );
  ...
  if (input.defaultPortrait) {
    ...
    statements.push(
      database
        .prepare(`DELETE FROM character_default_portraits WHERE character_id=?`)
        .bind(input.characterId),
    );
  }
  return statements;
...
export function parseCharacterPortraitLibraryForm(form: FormData): {
  ...
  if (!Array.isArray(parsedSheetIds))
    throw new HttpError(400, "角色脸图配置格式不合法");
  const faceSheetIds = parsedSheetIds.map((value) => Number(value));
  if (
    faceSheetIds.some((value) => !Number.isSafeInteger(value) || value <= 0)
  ) {
