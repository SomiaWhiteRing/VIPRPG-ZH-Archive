...
type Context = { params: { characterId: string } };

async function requireCharacter(runtime: AppRuntime, context: Context) {
...
export async function GET(
  ...
  try {
    await requireCharacter(runtime, context);
    const params = new URL(request.url).searchParams;
    if (!params.has("blob")) {
      const page = await searchCharacterMaterialsForAdmin(runtime, {
        kind: params.get("kind") ?? "faceset",
        query: params.get("q") ?? "",
    ...
    if (!/^[a-f0-9]{64}$/.test(hash))
      throw new HttpError(400, "素材地址不合法");
    const row = await getD1(runtime)
      .prepare(
        `SELECT content_type_hint AS type,size_bytes AS size FROM blobs b
    ...
    const object = await getBlob(runtime, hash);
...
export async function POST(
  ...
  if ("response" in auth) return auth.response;
  try {
    const characterId = await requireCharacter(runtime, context);
    const form = await request.formData();
    const kind = String(form.get("kind"));
    if (!["charset", "monster", "other"].includes(kind))
      throw new HttpError(400, "请选择素材分类");
    const file = form.get("image");
    if (!(file instanceof File) || !file.size || file.size > 2 * 1024 * 1024)
      throw new HttpError(400, "请选择不超过 2 MiB 的图片");
    let info;
    try {
      info = inspectForumImage(await file.arrayBuffer());
    } catch (error) {
      if (error instanceof ForumImageValidationError)
    ...
    if (
      info.width > 8192 ||
      info.height > 8192 ||
      info.width * info.height > 16_777_216
    )
      throw new HttpError(
        400,
        "图片尺寸过大，最长边不能超过 8192 像素，总像素不能超过 1600 万",
      );
    const [hash] = await storeWorkImages(runtime, [
    ...
    await db.batch([
      db
        ...
        .bind(hash, kind, info.width, info.height),
      db
        .prepare(
          `INSERT OR IGNORE INTO character_material_bindings(character_id,material_id)
    ...
    const material = await db
      ...
      FROM character_materials WHERE blob_sha256=? AND kind=?`,
      )
      .bind(hash, kind)
...
export async function PUT(
  ...
  try {
    ...
    const form = await request.formData();
    if (
      !["face_sheet_ids", "default_portrait", "material_ids"].every((key) =>
        form.has(key),
      )
    )
      throw new HttpError(400, "素材配置不完整，请刷新重试");
    const portrait = parseCharacterPortraitLibraryForm(form);
    let parsed: unknown;
    try {
      parsed = JSON.parse(String(form.get("material_ids")));
    } catch {
      throw new HttpError(400, "素材配置格式不合法");
    }
    if (
      ...
      throw new HttpError(400, "素材 ID 不合法");
    const ids = [...new Set(parsed as number[])];
    const db = getD1(runtime);
    const selected = JSON.stringify(ids);
    const count = await db
      .prepare(
        `SELECT count(*) AS count FROM character_materials m JOIN blobs b ON b.sha256=m.blob_sha256
      WHERE m.id IN (SELECT value FROM json_each(?)) AND b.status='active'`,
      )
      .bind(selected)
      .first<{ count: number }>();
    if (count?.count !== ids.length)
      throw new HttpError(400, "部分素材已不可用，请刷新后重试");
    const statements = await prepareCharacterPortraitLibraryUpdate(runtime, {
      ...portrait,
      characterId,
    ...
    statements.push(
      db
        ...
        .bind(characterId, selected),
      db
        .prepare(
          `DELETE FROM character_material_bindings WHERE character_id=? AND material_id NOT IN (SELECT value FROM json_each(?))`,
        )
        .bind(characterId, selected),
      db
        .prepare(
          `UPDATE character_material_bindings SET sort_order=(
        SELECT CAST(key AS INTEGER) FROM json_each(?) WHERE value=material_id
      ) WHERE character_id=?`,
        )
        .bind(selected, characterId),
      db
        .prepare(
          `UPDATE character_face_sheet_bindings SET sort_order=(
