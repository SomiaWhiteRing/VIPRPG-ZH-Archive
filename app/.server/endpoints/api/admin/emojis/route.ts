...
import { insertBlobRecord } from "@/app/.server/db/archive-objects";
...
export async function GET(runtime: AppRuntime, request: Request) {
  const auth = await requirePermission(runtime, request, "custom_emoji.manage");
  if ("response" in auth) return auth.response;
  try {
    return json({ ok: true, emojis: await listAdminEmojis(runtime) });
  } catch (error) {
...
export async function POST(runtime: AppRuntime, request: Request) {
  ...
  try {
    ...
    if (!ALLOWED_TYPES.has(file.type.toLowerCase()))
      throw new HttpError(400, "只支持 PNG、JPEG、GIF 或 WebP 图片");
    if (file.size <= 0 || file.size > MAX_EMOJI_BYTES)
      throw new HttpError(400, "表情图片大小不合法");
    ...
    if (existing?.status !== "active") {
      await putBlob(runtime, sha256, body, body.byteLength, file.type);
    ...
    const emoji = await createCustomEmoji(runtime, {
      shortcode: String(form.get("shortcode") ?? ""),
      name: String(form.get("name") ?? ""),
      category: String(form.get("category") ?? "站点"),
      visibleInPicker:
        String(form.get("visibleInPicker") ?? "true") !== "false",
...
export async function PATCH(runtime: AppRuntime, request: Request) {
  const auth = await requirePermission(runtime, request, "custom_emoji.manage");
  if ("response" in auth) return auth.response;
  try {
    ...
    if (!Number.isSafeInteger(id) || id <= 0)
      throw new HttpError(400, "表情 ID 不合法");
    const status =
    ...
    const emoji = await updateCustomEmoji(runtime, id, {
      name: body.name === undefined ? undefined : String(body.name),
      category: body.category === undefined ? undefined : String(body.category),
      visibleInPicker:
        body.visibleInPicker === undefined
          ? undefined
          : Boolean(body.visibleInPicker),
      status,
    });
