...
export default async function FavoritesPage({ searchParams }: { searchParams: Promise<{ page?: string | string[] }> }) {
  ...
  return <div><PageHeader title="收藏" subtitle={`共 ${result.total} 部作品`} />{result.items.length ? <FavoriteGrid items={result.items} /> : <AccountEmpty>还没有收藏作品。</AccountEmpty>}<PaginationLinks basePath="/me/favorites" page={page} hasNext={page * 20 < result.total} /></div>;
}
