  ...
  return (
    <article className="flex items-start gap-3.5 py-3.5">
      <Link
        ...
        href={`/games/${work.id}`}
      >
        <WorkThumbnail blobSha256={work.previewBlobSha256} alt={title} width={128} height={96} imageClassName="h-full w-full object-cover transition-transform group-hover:scale-[1.02]" fallbackClassName="grid h-full place-items-center px-1 text-center font-mono text-[10.5px] text-muted" fallback={engineLabel(work.engineFamily)} />
      </Link>
      <WorkListSummary href={`/games/${work.id}`} title={title} originalTitle={work.chineseTitle ? work.originalTitle : null} releaseDate={work.originalReleaseDate} engineFamily={work.engineFamily} language={work.language} truncateOriginal></WorkListSummary>
      {download ? (
        <a
