...
- 上传者和自定义角色可由超级管理员设置“开放申请”和“向所有用户开放”，两个开关独立保存。管理员允许开放申请但禁止全员授予；超级管理员与基础用户均不开放。当前申请组及默认配置见[可申请角色配置](requestable-roles.md)。内置角色的说明可编辑，身份、名称、优先级、状态和功能定义保持系统约束。
- `user_roles` 只记录单独授予的角色；`effective_user_roles` 汇合启用的个人角色与正常账户可用的全员开放角色，身份读取、权限检查和管理优先级均使用这一口径。全员开放包含以后注册的账户，收回时保留单独授权；不为全员开放批量创建个人关系或角色事件。
...
- 角色资料与功能权限分区保存；服务端在写入批次的末尾返回完整已保存角色，前端据此更新比较基准并保留另一区未提交的草稿。资料表单显式提交，不在保存后重置控件，避免将开放开关恢复为初始值。
...
- 作者详情卡片下方提供前台编辑入口，`creator.metadata.update_public` 允许正常账号维护已关联公开作品的作者。普通用户和上传者不再固定获得此功能，由自定义角色 `creator_editor`（维护作者信息）授予；当前默认开放申请、关闭全员开放，同时配置后台作者读取和编辑权限。管理员及拥有 `creator.metadata.update_any` 的其他角色继续可编辑。单项公开编辑权限仅允许编辑名称、别名、网站、简介和头像，不隐式授予后台访问、非公开作者读取或合并能力。入口、编辑页、资料与头像 API 及写入批次均检查当前生效权限，收回全员开放后保留单独角色授权。别名使用共用多 tag 输入器，不设数量上限。网站以 `creators.links_json` 保存有序的 `{label,url}` 数组，支持 itch、Twitter(X)、blog、个人网站及自定义名称，每条网址只允许 HTTP(S)。资料直接生效，保存时原子比对原始资料，冲突返回 409；头像独立保存并比对原头像。修改人与修改前后内容随资料一起写入审计日志，名称及别名冲突时整批回滚。
...
- 管理员角色的授予、收回及申请审批仅限当前超级管理员；管理员允许开放申请，管理员和超级管理员均禁止全员授予。判断是否已拥有管理员角色时核对实际管理员或根身份，不以自定义角色覆盖其 permission 清单代替身份。
...
- 个人中心 `/me/permissions` 按角色展示名称和介绍，不列出具体功能，通过 `POST /api/account/role-requests` 提交 `{roleId, reason?}`。理由为最多 2000 字的选填纯文本，只对本人和管理员可见。拥有该角色或已有权限覆盖全部功能、全员开放、待审核、关闭申请或停用时隐藏申请表单。
- 申请复用一条共享 `role_change_request` 提醒；管理员申请仅申请人和当前超级管理员可见，其余申请对申请人及所有当前管理员（含超级管理员）可见。普通自定义角色即使包含角色分配或审批 permission，也不获得申请收件和审批资格。各管理员独立保存已读状态。
- 同一用户对同一角色最多一条待审申请，重复提交复用原记录。首个有效批准或驳回结束本次申请，批准同时单独授予角色；申请、角色事件、通知和审计原子提交。审批继续检查双方层级，禁止自我审批。
...
- 正式站 `https://viprpg.org` 注册只授予基础角色，不按注册顺序自动产生超级管理员。负责人自行完成邮箱验证与注册后，通过 `scripts/rotate-bootstrap-admin.mjs --production --email <邮箱>` 预览，并在确认目标后以 `--apply --confirm <邮箱>` 初始化根权限；该操作保留角色事件和审计，并撤销受影响会话，需要重新登录。开发和预生产保留首个验证注册账号自动获得 `super_admin` 的行为；已有超级管理员时不重复授予。
