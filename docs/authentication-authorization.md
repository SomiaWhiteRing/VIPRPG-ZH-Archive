...
- 上传者和自定义角色可由超级管理员设置“开放申请”和“向所有用户开放”，两个开关独立保存。默认仅上传者开放申请，全员开放默认关闭。内置角色的说明可编辑，身份、名称、优先级、状态和功能定义保持系统约束。
...
- 个人中心 `/me/permissions` 按角色展示说明及实际功能清单，通过 `POST /api/account/role-requests` 提交 `{roleId, reason?}`。理由为最多 2000 字的选填纯文本，只对本人和管理员可见。拥有该角色或已有权限覆盖全部功能、全员开放、待审核、关闭申请或停用时隐藏申请表单。
