...
- 上传者和自定义角色可由超级管理员设置“开放申请”和“向所有用户开放”，两个开关独立保存。默认上传者开放申请；初始化的自定义“作者资料编辑”角色默认开放申请且全员开放，其他角色全员开放默认关闭。内置角色的说明可编辑，身份、名称、优先级、状态和功能定义保持系统约束。
...
- 作者详情卡片下方提供前台编辑入口，`creator.metadata.update_public` 允许正常账号维护已关联公开作品的作者。普通用户和上传者不再固定获得此功能，由自定义角色 `creator_editor`（作者资料编辑）授予；默认全员开放，超级管理员可收回或改成申请审批、单独授予。管理员及拥有 `creator.metadata.update_any` 的其他角色继续可编辑。前台仅编辑名称、别名、网站、简介和头像，不授予后台访问、非公开作者读取或合并能力。入口、编辑页、资料与头像 API 及写入批次均检查当前生效权限，收回全员开放后保留单独角色授权。别名使用共用多 tag 输入器，不设数量上限。网站以 `creators.links_json` 保存有序的 `{label,url}` 数组，支持 itch、Twitter(X)、blog、个人网站及自定义名称，每条网址只允许 HTTP(S)。资料直接生效，保存时原子比对原始资料，冲突返回 409；头像独立保存并比对原头像。修改人与修改前后内容随资料一起写入审计日志，名称及别名冲突时整批回滚。
...
- 管理员角色的授予与收回仅限当前超级管理员；管理员和超级管理员均禁止开放申请或全员授予。
...
- 申请复用一条共享 `role_change_request` 提醒，申请人及所有当前管理员（含超级管理员）可见。普通自定义角色即使包含角色分配或审批 permission，也不获得申请收件和审批资格。各管理员独立保存已读状态。
