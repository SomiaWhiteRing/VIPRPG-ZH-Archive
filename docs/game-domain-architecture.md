...
```text
Series 管一组 Work；Work 管资料；Release 管公开版本；ArchiveVersion 管本站保存的文件快照。
```
...
- 发布时间是什么？
- 来源链接是什么？
- 是否校对、修图、使用 Maniacs Patch？
- 可执行入口是什么？

Release 不等同于独立补丁包。架构仍然不以“补丁包增量继承”作为归档目标。这里的 Release 指的是一份完整可运行发布物的资料层。
...
ArchiveVersion 发布后应当不可变。若需要修正，应创建新的 ArchiveVersion，并把 Release 的当前下载指向新快照。
...
- `works.primary_title` 是站内主要展示名。
- `work_titles` 用来保存原名、译名、别名、罗马字、缩写，避免把多个标题塞进同一字段。
...
- `releases` 负责玩家能理解的版本资料。
- `archive_versions` 负责本站导入和下载重组需要的技术快照。
- `patch_applied_full_release` 表示“已经打好补丁的完整发布物”，不是独立补丁包支持。
- `is_current` 表示某个 Release 当前默认下载的归档快照。
...
- 作者可同时挂在 Work 和 Release 上。原作者属于 Work，汉化、校对、修图、再打包人员通常属于 Release。
...
5. 前端调用 preflight，询问哪些 blob / core pack 已存在。
6. 前端只上传缺失 blob 和本次 core pack。
7. 前端提交 ArchiveVersion commit。
8. 服务端写入 `archive_versions`、`archive_version_files`、manifest R2 对象和统计信息。
9. 管理员或有权限用户发布 Work / Release / ArchiveVersion。
...
公开下载不应该直接使用 Work 或 Release 的文件字段，而是从 Release 当前指向的 ArchiveVersion 开始。
...
GET /games/{work_slug}/releases/{release_id}/download
  -> 查询 release
  -> 找到 is_current = 1 的 published archive_version
  -> 读取 manifest
...
当用户从作品页点下载时，页面使用当前 Release 的 current ArchiveVersion。管理员需要复现旧快照时，可以从归档管理页指定 ArchiveVersion 下载。
...
ORDER BY ws.position_number, ws.position_label, w.sort_title, w.primary_title;
```
...
WHERE wc.character_id = ?
  AND w.status = 'published'
ORDER BY wc.role_key, w.primary_title;
```
...
- Work 的 slug 全局唯一。
- 同一 Work 下 Release label 唯一。
- 同一 Release 下 ArchiveVersion label 唯一。
- 同一 Release 同时只能有一个 published current ArchiveVersion。
...
- 一个作品可以有多个 Release。
- 一个 Release 可以有多个 ArchiveVersion，但只有一个 current。
- 上传一个新版本时，默认创建 Release；若只是重新导入同一发布物，创建 ArchiveVersion。
- 活动投下信息挂在 Release，不挂在 Work。
- 作者原作者挂在 Work，汉化、校对、修图、再打包人员挂在 Release。
- 主要登场角色挂在 Work。
