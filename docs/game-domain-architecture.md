...
上传或由上传者保存作品资料时，原始发布日期必填；允许 `yyyy`、`yyyy-MM`、`yyyy-MM-dd` 三种精度，并按实际精度保存，不补全未知的月份或日期。
...
- 作品、作者、角色和游玩页的短评论楼中楼默认随主楼返回并按创建时间正序展示前 5 条；超过 5 条显示“查看全部 N 条回复”。展开后在楼内每页展示 10 条，支持上一页、下一页和收起；收起恢复前 5 条，不影响主楼分页。数量与列表使用相同公开条件，包含删除占位；新增回复后自动展开并定位其实际所在页，加载失败保留当前内容并允许重试。
...
- 公开作者列表使用桌面两列、窄屏单列的轻量名片，展示头像、名称、别名、可选简介与去重后的公开参与作品数，不展示关联作品名称、封面或链接。未设置头像时沿用作者头像占位规则。仅包含至少参与一部 published Work 的人物；名称与别名均可搜索，命中别名优先展示。默认按名称排序，也可按公开作品数降序排列，每页 30 位，总数使用相同公开与搜索条件独立统计。
...
- 作者和标签索引只显示有关联 published Work 的条目；公开目录及其成员各自遵守 published 条件。角色身份独立公开，没有公开作品也可搜索或打开详情；分类浏览按[角色分类](./character-index.md)中的归属规则展示。
