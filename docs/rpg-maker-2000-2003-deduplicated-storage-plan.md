...
CRC32 不是内容身份，但下载 ZIP 的 local file header 需要在写出文件数据前知道 CRC 和大小。因此浏览器预索引阶段会为每个文件计算 CRC32，并把它写入 manifest 和 `archive_version_files.crc32`。下载端用它构建标准 STORE ZIP；Web Play 用它生成 pack 切片索引和后续校验。
...
- `archive_version_files.crc32`：逻辑文件本身的 CRC32，仅用于 STORE ZIP local header、普通下载兼容性和 Web Play pack 切片校验；不参与内容寻址。
...
3. 最终清理：对已放入回收站且超过目标宽限期的 ArchiveVersion 写入 `purged_at`，删除 `archive_version_files` 和 manifest R2 对象；最终清理后不能还原。
...
- 回收站最终清理会写入 `archive_versions.purged_at`，删除其 `archive_version_files` 记录，并删除对应 manifest R2 对象。最终清理后只能保留基础审计和显示记录，不能再还原。
- 最终清理中的 GC sweep 只处理 `blobs.status = 'active'` / `core_packs.status = 'active'`、超过目标宽限期、且没有任何 `archive_version_files` 引用的对象。
...
- 新增管理员专用 GC sweep API：必须显式提交 `confirm = SWEEP`，先最终清理超过目标宽限期的回收站 ArchiveVersion，再清理零引用对象；默认每类最多清理 1000 个超过 7 天的对象，管理员手动执行时可指定 `graceDays = 0` 立即清理。sweep 会删除 R2 blob/core pack，并把 D1 对象状态改为 `purged`；失败对象恢复 `active`。
