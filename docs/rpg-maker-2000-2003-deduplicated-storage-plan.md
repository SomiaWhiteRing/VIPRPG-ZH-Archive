...
- `RPG_RT.exe` 和 DLL 不从 Web Play ZIP 中剔除；OPFS 中的本地安装目录与下载 ZIP 内容保持一致。
- ZIP 解包完成后不长期保存，浏览器本地只保留解包后的游戏目录、安装状态和 EasyRPG 存档。
...
- ZIP 只在浏览器下载和解包过程中临时存在，解包后丢弃；OPFS 中保留完整游戏目录。
- OPFS、IndexedDB、Cache API 和 EasyRPG IDBFS 共享同一个 origin 的浏览器存储额度；进入安装流程前应调用 `navigator.storage.estimate()` 和 `navigator.storage.persist()` 并把结果展示给用户。
- IndexedDB 负责安装状态、锁、版本键、进度、文件清单和错误；OPFS 负责游戏文件本体；EasyRPG IDBFS 负责存档。
...
- 安装过程必须有锁和崩溃恢复；安装完成前不能启动 EasyRPG。
- `index.json` 由 archive manifest 或 ZIP entry 列表生成，不能依赖上传者提供。
...
- Web Worker 流式解 ZIP，写入 OPFS，生成 EasyRPG `index.json`。
- IndexedDB 记录安装状态、锁、版本键、文件清单、进度、错误和最后游玩时间。
- Service Worker 拦截 `/play/games/{playKey}/...`，从 OPFS 返回文件。
- UI 提供安装中拦截关闭、安装失败清理、删除本地缓存、重新安装和日志面板。
...
- 游戏文件安装目录使用 OPFS，表达“本站私有的本地游戏目录”。
- 安装状态、锁、进度和文件清单使用 IndexedDB。
...
- 安装前调用 `navigator.storage.persist()` 请求持久化存储，并把结果展示给用户。
