...
2026-05-01 对 `D:\path\to\game-folder` 做了一次 staging 受控导入。当前导入策略为 `file_policy_version = 'rpgm2000-2003-whitelist-v3'`；v3 相比当时样本规则额外排除根目录 `null.txt`。该样本根目录未发现 `null.txt`，所以下列统计不变。
...
下载 ZIP Workers Cache/CDN 命中
  ...
  -> 浏览器端下载 ZIP 到 Web Worker 内存
  -> 按 ZIP 中央目录逐文件写入 OPFS
  -> OPFS 保存 Web Play 运行目录（跳过 .txt / .exe / .dll）
...
- ZIP 解包完成后不长期保存，浏览器本地只保留解包后的 Web Play 运行目录、安装状态和 EasyRPG 存档。当前下载 ZIP 是 STORE + data descriptor 形态，Web Play 安装器先把 ZIP 字节留在 Web Worker 内存中，再按中央目录定位 entry，并通过滚动窗口式并发 OPFS 写入、目录 handle cache 和 IndexedDB 批量提交加速小文件安装；`.txt`、`.exe`、`.dll` entry 不写入 OPFS、不进入 EasyRPG 索引，ZIP 不写入 OPFS、IndexedDB 或 Cache API。
...
GET /play/{archiveVersionId}
  ...
  -> Web Worker 下载 ZIP 字节
  -> 按 ZIP 中央目录逐文件写入 OPFS
  -> 生成 EasyRPG index.json
  -> IndexedDB 标记安装 ready
  -> 加载同源 EasyRPG runtime
  -> Service Worker 从 OPFS 响应 /play/games/{playKey}/{path...}
```

固定边界：

- 在线游玩必须复用现有下载 ZIP 的 URL、字节内容和缓存 key；不得为了 Web Player 排除 `exe` / DLL 或生成另一份 ZIP。
- ZIP 只在浏览器下载和解包过程中临时存在，解包后丢弃；OPFS 中保留 Web Play 运行目录，跳过所有 `.txt`、`.exe`、`.dll` 文件。由于当前 ZIP builder 使用 STORE + data descriptor，浏览器安装器以 Web Worker 内存中的 ZIP 中央目录为索引，不采用流式 unzip 库直接解包。
...
- Service Worker 只读 OPFS 文件并返回 Response；MVP 不做单文件云端 fallback，避免本地安装后仍持续消耗 R2。
...
- 已建立 staging D1/R2 和 `works` / `releases` / `archive_versions` / `archive_version_files` 数据模型。
- 已实现单个 blob、core pack 上传校验和 `/api/imports/preflight`。
- 已新增 `tools/rpgm-archive-importer/` 作为 Phase C 受控导入工具。
- 已用 `本地样本游戏` 完成 staging 试导入，证明 manifest、core pack、blob、D1 索引和排除统计链路可运行。
...
- 管理端提供手动按钮触发一致性检查和 GC dry-run，避免定时任务尚未实现时只能直接访问 API。
...
- 新增管理员专用 GC dry-run API：只报告候选对象，不执行删除。
- MVP 的可清理候选定义为：`status = active`、超过宽限期、且没有任何 `archive_version_files` 引用的 blob/core pack。
- 仅被 `deleted` ArchiveVersion 引用的对象单独统计，暂不自动删除；后续必须先明确 ArchiveVersion purge 流程，再允许真实 sweep。
...
- Web Worker 下载 ZIP，解析中央目录，跳过 `.txt`、`.exe`、`.dll` 文件，滚动窗口式并发写入 OPFS，生成 EasyRPG `index.json`。
- IndexedDB 记录安装状态、版本键、文件清单、进度、错误和最后游玩时间。MVP 将刷新或崩溃遗留的 `installing` 状态视为中断安装，并要求用户清理重装；跨标签页强锁后续再补。
- Service Worker 拦截 `/play/games/{playKey}/...`，从 OPFS 返回文件。
...
- OPFS 中存在 Web Play 运行目录和生成的 `index.json`；本地运行目录不包含 `.txt`、`.exe`、`.dll` 文件。
...
- 游戏文件安装目录使用 OPFS，表达“本站私有的 Web Play 运行目录”；该目录跳过 `.txt`、`.exe`、`.dll` 文件，普通下载 ZIP 仍保留这些文件。
