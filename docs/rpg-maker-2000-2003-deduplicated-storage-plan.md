...
当前 `file_policy_version = 'rpgm2000-2003-whitelist-v2'` 的允许类型：
...
> 注意：这一全量基线来自文件类型白名单扫描，尚未对全部样本重跑 `rpgm2000-2003-whitelist-v2` 的路径覆盖规则。`whitelist-v2` 的效果先以 2.7 的单游戏样本作为实测依据；后续批量导入前应重新生成全量 v2 基线。
...
2026-05-01 对 `D:\path\to\game-folder` 做了一次 staging 受控导入，使用 `file_policy_version = 'rpgm2000-2003-whitelist-v2'`。
...
- `screenshots/` 目录和根目录 `screenshot*` 文件均未进入 manifest。
...
```json
{
  ...
  "archiveVersion": {
    ...
    "filePolicyVersion": "rpgm2000-2003-whitelist-v2",
