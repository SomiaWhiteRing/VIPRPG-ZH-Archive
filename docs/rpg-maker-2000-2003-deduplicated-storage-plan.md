...
```text
GET /downloads/{game_version_id}/{manifest_sha256}/{packer_version}.zip
...
POST /api/games/{id}/versions/{versionId}/download-jobs
POST /api/games/{id}/versions/{versionId}/warm-edge-cache
```
...
- 发信优先使用 Cloudflare Email Service 的 Workers Email binding。若 Email Service 不满足投递率、模板或运营需求，再接入 Resend、Postmark 等第三方邮件服务。
- 邮件发件域必须使用项目自有域名；`EMAIL_FROM` 固定为已验证 sender，不允许请求参数覆盖发件人。
- 注册邮件只包含验证码、过期时间和安全提示；找回密码邮件只包含验证码、过期时间和安全提示；邮件不包含上传权限审批状态等后台信息。
- 邮箱验证只证明账户归属，不等于上传权限；非管理员用户仍必须由管理员审核为 `approved uploader` 后才能上传。
- 管理员 bootstrap 仍可使用 `BOOTSTRAP_ADMIN_EMAIL`，但正式系统中该邮箱也应设置密码并完成邮箱验证；bootstrap 只决定初始角色，不绕过长期登录安全。
