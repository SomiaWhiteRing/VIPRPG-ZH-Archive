...
`available` 响应包括本站 version、releaseId、releaseSequence、selectionRevision、publishedAt、notes、同源 notesUrl（指向链接卡片）、文件固定地址／长度／摘要／格式。版本和更新说明由后台填写，客户端读取 `notes`；文件地址从本站 origin 配置生成。已配置但暂停返回 `paused`；未配置返回 404，存储缺失或校验异常返回错误，不能冒充“已是最新版”。JSON 不缓存。
