# VIPRPG 中文归档项目文档入口

## 架构文档

- [RPG Maker 2000/2003 去重存储架构](./rpg-maker-2000-2003-deduplicated-storage-plan.md)
- [OpenNext 应用与 Cloudflare 基础设施开发路径](./opennext-cloudflare-development-path.md)
- [游戏领域架构设计](./game-domain-architecture.md)
- [浏览器上传任务架构设计](./browser-upload-task-architecture.md)
- [EasyRPG 在线游玩架构设计](./easyrpg-web-play-architecture.md)

## 问题修正归档

这类文档记录已经发现的架构缺陷、修正方案、验证结果和后续风险。它们不是新的主架构入口；结论落地后，应同步回上面的正式架构文档。

- [D1 文件行下放计划](./bugfixes/d1-file-rows-offload-plan.md)
