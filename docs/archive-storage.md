...
浏览器哈希完成后默认进行[原版 RTP 引用检查](./rtp-cleanup.md)，仅清理目录、名称和内容指纹均命中已知清单且确认未引用的 RTP。非 RTP 不参与此项清理，分析不确定时保留。排除记录随 manifest 和恢复草稿保存，source-ready 复用核心 ZIP 校验复核引用；用户可在选择文件前关闭。
