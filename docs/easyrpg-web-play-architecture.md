...
- 移动端运行时默认显示十字键、A（键盘 Z，确认）和 B（键盘 X，取消／菜单）。Shift、Menu（F1）、Debug（F9）、log（反引号键）默认隐藏，可在布局编辑中各自添加或删除。十字键可滑动换向、组合斜方向，也可与操作键同时按住；输入通过会话接口转成 iframe 内 SDL3 接收的键盘事件。抬手、触控取消、失去捕获、窗口失焦、后台切换、方向／布局模式切换和会话销毁时释放相应输入。
- 移动端横屏时，方向切换、恢复窗口、截图和布局设置按钮位于左上纵列；竖屏保持右上横排。默认控制器布局参考 GBA／GBA SP：十字键在左，A 在右上、B 在左下，横屏分列画面两侧，竖屏放在画面下方。游戏画面默认靠上，保持 4:3；移动端竖屏窗口使用较高的容器容纳控制器。虚拟按钮使用纯色，不使用毛玻璃效果。
- 横竖屏均可编辑按钮布局。十字键整体移动，A、B 和每个可选功能键均独立定位；可在整个可用区域自由拖动，每个控件分别调整大小（50%–200%）和透明度（0%–100%）。只有游戏画面限制为竖屏上下移动，横屏画面位置固定。控件大小和位置会限制在安全区域内；编辑时可收起属性面板，拖动期间临时隐藏面板，完全透明的按钮仍有编辑边框和名称。
