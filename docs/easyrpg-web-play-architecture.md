...
- 运行中：桌面端在原启动位置并列显示“网页全屏”“全屏幕”和截取图片按钮；移动端隐藏“网页全屏”。网页全屏让 iframe 铺满浏览器视口，在右上角显示恢复按钮；桌面端 canvas 保持 4:3 居中。全屏幕优先使用外层播放器容器的浏览器原生 fullscreen；浏览器拒绝时改为页面铺满，不调用 EasyRPG runtime 自带 fullscreen。移动端只有一个方向切换按钮，显示可切换到的方向；恢复窗口不会重建 iframe 或重启游戏。
- 移动端运行时显示方向键、Z（确认）、X（取消／菜单）和 Shift。方向键可滑动换向、组合斜方向，也可与操作键同时按住；输入通过会话接口转成 iframe 内 SDL3 接收的键盘事件。抬手、触控取消、失去捕获、窗口失焦、后台切换、方向／布局模式切换和会话销毁时释放相应输入。
- 移动端横屏时，方向切换、恢复窗口和截图按钮位于左上纵列；竖屏保持右上横排，并提供布局设置。竖屏游戏画面默认位于工具栏下方，方向键组与操作键组位于底部；移动端竖屏窗口使用较高的容器容纳控制器，游戏画面仍保持 4:3。
- 布局设置仅在竖屏开放。可分别上下拖动游戏画面、方向键组和操作键组，不能左右移动或拖出安全区域；支持保存、取消和恢复默认。编辑时拦截游戏触控；切换方向或恢复窗口取消未保存的调整。方向偏好和布局统一保存在 `localStorage` 的 `viprpg:web-play:controls` 中，`layouts.portrait`、`layouts.landscape` 分开保存；位置用各元素可移动距离的比例表示，适应视口变化。布局编辑的拖动与边界限制方式参考 [melonDS Android 的布局编辑器](https://github.com/rafaelvcaetano/melonDS-android/blob/master/app/src/main/java/me/magnum/melonds/ui/layouteditor/LayoutEditorView.kt)。
...
- 游戏画面保持 4:3；桌面端默认窗口游玩，启动后可分别进入网页全屏和全屏幕；移动端按缓存方向全屏，显示虚拟控制器，并提供单个横竖屏切换按钮与恢复窗口。横屏工具栏位于左上纵列，竖屏画面默认靠上且可调整垂直布局。
