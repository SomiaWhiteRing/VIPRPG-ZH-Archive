...
AUTH_SECRET 等 Worker runtime secrets 不由 workflow 写入；正式与 staging 分开设置。正式域名、邮件、资源初始化和根账户注册先按[正式部署手册](./production-deployment.md#首次正式初始化)准备并确认。正式站沿用首个验证注册账号自动获得 `super_admin` 的行为，已有超级管理员时不重复授予；正常注册不增加运维确认步骤。
