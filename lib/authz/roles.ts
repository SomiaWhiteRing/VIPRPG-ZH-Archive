...
export const ROLE_TEMPLATES = {
  wiki_editor: {
    key: "wiki_editor",
    name: "维基人",
    description: "维护作品、作者、角色、标签及作品关联资料；允许移除错误关联，不授予作品发布、实体删除、归档文件、维护者或账户管理能力。",
    priority: 300,
    permissionKeys: [
...
export function roleEditSnapshot(role: {
  name: string; description: string; priority: number; status: RoleStatus; permissionKeys: readonly string[];
}): string {
  return JSON.stringify([role.name, role.description, role.priority, role.status, [...role.permissionKeys].sort()]);
}
