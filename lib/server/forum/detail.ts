import { FORUM_POST_PAGE_SIZE, type ForumDetail } from "@/lib/forum";
import { interactiveContent, interactiveTopic } from "@/lib/forum-state";
import type { ArchiveUser } from "../db/user-access";
import type { CustomEmojiDto } from "../db/work-community";
import type { ForumRuntime } from "./runtime";
import { assertPublicTopic, forumPostPage, forumPreviews, publicCommentPage } from "./public-queries";
...
export async function forumDetail(
  ctx: ForumRuntime,
  topicId: number,
  input: { page: number; floor?: number; commentPage: number; comment?: number },
  user: ArchiveUser | null,
): Promise<ForumDetail> {
  let { page, floor, commentPage } = input;
  if (input.comment) {
    const location = await forumLocation(ctx, topicId, { commentId: input.comment });
    page = location.page;
  ...
  const topic = interactiveTopic(data.topic, engagement.viewer, engagement.topicCapabilities);
  ...
  const expanded = selected ? await publicCommentPage(ctx, selected.id, commentPage) : null;
  const posts = data.posts.items.map((post) => {
    ...
    const comments = post.id === selected?.id && expanded ? expanded : preview.comments;
    return {
  ...
  return { topic, posts: { ...data.posts, items: posts }, floor: floor ?? null, comment: input.comment ?? null };
}

export async function forumEmojis(ctx: ForumRuntime, bodies: (string | null)[]): Promise<CustomEmojiDto[]> {
  const codes = [...new Set(bodies.flatMap((body) =>
    [...(body ?? "").matchAll(/:([A-Za-z0-9_+\-]{1,64}):/g)].map((match) => match[1])))];
  ...
  const rows = await ctx.db.prepare(`SELECT id,shortcode,name,category,('/api/media/blobs/'||image_blob_sha256) AS imageUrl,
    ...
    .bind(JSON.stringify(codes)).all<CustomEmojiDto>();
  return rows.results;
}
