...
export async function GET(ctx: ForumRequestRuntime, request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params;
    const row = await ctx.db.prepare(`SELECT object_key,format,size,user_id,post_id,
    ...
    if (!row.public) {
      const user = (await loadRequestSession(ctx.db, request.headers.get("cookie")))?.user;
