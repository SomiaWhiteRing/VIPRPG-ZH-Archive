...
import { contentIdentity, rawTopic } from "@/lib/server/forum/queries";
...
async function boundedForm(request: Request) {
  ...
  try {
    return await new Response(await readForumBody(request, FORUM_IMAGE_BYTES + 16384), {
...
export async function POST(ctx: ForumRequestRuntime, request: Request) {
  try {
    ...
    const recent = await ctx.db.prepare(`SELECT COUNT(*) AS n FROM forum_images WHERE user_id=? AND datetime(updated_at)>=datetime('now','-1 minute')`)
      .bind(auth.user.id).first<{n:number}>();
    if ((recent?.n ?? 0) >= 12) throw new HttpError(429,"上传过于频繁，请稍后重试。","forum_image_rate_limit");
    ...
    if (file.size > FORUM_IMAGE_BYTES) throw new HttpError(413, "图片超过 2 MiB。");
