...
export function replyNotificationStatement(
  ctx: ForumRuntime, kind: "post" | "comment", actorId: number, token: string,
) {
  ...
  return ctx.db.prepare(`INSERT INTO inbox_items
...
export function likeNotificationStatement(ctx: ForumRuntime, actorId: number, postId: number) {
  return ctx.db.prepare(`INSERT INTO inbox_items
