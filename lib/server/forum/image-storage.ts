...
export type StoredForumImage = ForumImage & {
  ...
  object_key: string;
  fingerprint: string;
  status: "uploading" | "ready" | "failed" | "uncertain" | "cleanup" | "cleaned";
  updated_at: string;
};
...
export function imageUploadPending() {
  return new HttpError(409, "上传结果待确认，请一分钟后重试。已完成的图片会被保留。", "forum_image_pending");
}

function verifyObject(row: StoredForumImage, object: R2Object) {
  if (object.key !== row.object_key || object.size !== row.size ||
...
async function finishUpload(ctx: ForumRuntime, row: StoredForumImage): Promise<ForumImage> {
  ...
  const current = await db.prepare(`SELECT ${storedImageColumns} FROM forum_images WHERE id=?`)
    .bind(row.id).first<StoredForumImage>();
...
export async function reconcileForumImage(ctx: ForumRuntime, id: string) {
  ...
  try {
    ...
    await db.prepare(`UPDATE forum_images SET status='uncertain',updated_at=CURRENT_TIMESTAMP
      WHERE id=? AND status='uploading' AND updated_at=?`).bind(id, stamp).run();
    throw error;
  }
}
