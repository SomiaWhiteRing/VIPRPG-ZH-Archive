...
import { USER_ACCESS_COLUMNS, USER_ACCESS_JOINS, mapUserAccessRows, type SessionUserAccessRow } from "../db/user-access";

export async function loadRequestSession(db: D1Database, cookie: string | null) {
...
export async function findActiveSessionByHash(db: D1Database, hash: string) {
  const rows = await db.prepare(`SELECT s.id AS session_id,${USER_ACCESS_COLUMNS}
    ...
    ORDER BY r.priority DESC,r.id,rp.permission_key`).bind(hash).all<SessionUserAccessRow>();
