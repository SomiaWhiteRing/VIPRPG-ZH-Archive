...
export async function getWorkTranslators(workId: number): Promise<ConfirmedCreatorSelection[]> {
  const rows = await getD1().prepare(`SELECT c.id,c.name,ws.display_name FROM work_staff ws
    JOIN creators c ON c.id=ws.creator_id WHERE ws.work_id=? AND ws.role_key='translator' ORDER BY c.id`)
    .bind(workId).all<{ id: number; name: string; display_name: string }>();
  return (rows.results ?? []).map((row) => ({ kind: "existing", creatorId: row.id, name: row.name, displayName: row.display_name }));
}
...
export function parseTranslatorSelectionsJson(value: FormDataEntryValue | null): CreatorSelection[] {
  if (typeof value !== "string") throw new HttpError(400, "缺少译者资料。");
  let items: unknown;
  try { items = JSON.parse(value); } catch { throw new HttpError(400, "译者资料无法读取。"); }
...
export function parseWorkStaffJson(value: FormDataEntryValue | null): WorkStaffCreditInput[] {
  ...
  return items.map((item: unknown) => {
    ...
    const roleLabel = typeof item.roleLabel === "string" ? item.roleLabel.trim() || null : null;
    if (item.roleKey === "other" && !roleLabel) throw new HttpError(400, "请填写其他职务的名称。");
    return { selection: parseCreatorSelection(item.selection), roleKey: item.roleKey, roleLabel,
...
export async function prepareWorkStaffStatements(input: {
  ...
  const existingIds = [
    ...new Set(
      credits.flatMap((credit) =>
        credit.selection.kind === "existing" ? [credit.selection.creatorId] : [],
      ),
    ),
  ];
  ...
  for (const credit of credits) {
    ...
    if (seen.has(key)) throw new HttpError(400, "已有这条署名，请移除重复的制作人员职务。");
    seen.add(key);
