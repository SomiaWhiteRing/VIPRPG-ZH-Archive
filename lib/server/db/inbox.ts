...
import { INBOX_PAGE_SIZE, type InboxCategory } from "@/lib/inbox";
...
function inboxQuery(user: ArchiveUser) {
  ...
  return {
    sql: `WITH visible AS (${INBOX_SELECT} WHERE (${visibility.sql})), actionable AS (
      ...
      FROM visible)`,
    binds: [user.id,user.id,...visibility.permissionBinds,
      canResolveInboxRequests(user) ? 1 : 0,user.id,user.maxRolePriority,user.maxRolePriority],
  };
}

export function canResolveInboxRequests(user: ArchiveUser) {
  return hasPermission(user,"inbox.role_request.resolve") && hasPermission(user,"user.role.assign");
}

export async function listInboxItemsForUser(user: ArchiveUser, input: {
  category: InboxCategory; unread: boolean; page: number;
}) {
  const query = inboxQuery(user);
  const categorySql = {
    all: "1", replies: "type='forum_reply'", likes: "type='forum_like'",
    system: "type IN ('role_change_request','role_change_notice','system_notice')",
    pending: "can_reject=1",
  }[input.category];
  const filter = `${categorySql} AND ${input.unread ? "read_at IS NULL" : "1"}`;
  const totals = await getD1().prepare(`${query.sql} SELECT
    ...
    .bind(...query.binds).first<{total:number;pending:number;unread:number}>();
  const total = totals?.total ?? 0;
  const page = Math.max(1,Math.min(input.page,Math.max(1,Math.ceil(total/INBOX_PAGE_SIZE))));
  const rows = await getD1().prepare(`${query.sql} SELECT * FROM actionable WHERE ${filter}
    ORDER BY created_at DESC,id DESC LIMIT ? OFFSET ?`)
    .bind(...query.binds,INBOX_PAGE_SIZE,(page-1)*INBOX_PAGE_SIZE).all<InboxItemRow>();
  const items = (rows.results ?? []).map(mapInboxItemRow);
  await attachInteractions(items);
  return {items,total,page,pageSize:INBOX_PAGE_SIZE,pending:totals?.pending ?? 0,unread:totals?.unread ?? 0};
}
...
export async function markInboxItemRead(input: {
  ...
  const item = await getInboxItemForUser(input.itemId, input.user);
  if (input.target && (!item.interaction ||
...
export async function markAllInboxItemsRead(user: ArchiveUser): Promise<number> {
  ...
  const result = await getD1().prepare(`INSERT INTO inbox_item_reads(item_id,user_id,read_at)
    ...
    ON CONFLICT(item_id,user_id) DO NOTHING`)
    .bind(user.id,user.id,...visibility.permissionBinds,user.id).run();
...
export async function getInboxItemForUser(
  ...
  await attachInteractions([item]);
  return item;
}

function mapInboxItemRow(row: InboxItemRow): InboxItem {
  return {
    canApprove: !!row.can_reject && row.role_status === "active" && !row.already_assigned,
...
async function attachInteractions(items: InboxItem[]) {
  const interactions = items.filter((item) => item.type === "forum_reply" || item.type === "forum_like");
  if (!interactions.length) return;
  type InteractionRow = {
    id:number; topic_id:number; post_number:number; comment_id:number|null; topic_title:string;
    excerpt:string; actor_id:number; actor_name:string; actor_status:string; actor_avatar:string|null;
    reply_to_id:number|null;
  };
  const rows = await getD1().prepare(`SELECT i.id,t.id AS topic_id,p.post_number,c.id AS comment_id,
    ...
    .bind(JSON.stringify(interactions.map((item) => item.id))).all<InteractionRow>();
  ...
  for (const item of interactions) {
    ...
    const action = item.type === "forum_like" ? "赞了你的帖子"
      : row.comment_id ? row.reply_to_id ? "回复了你的回复" : "回复了你的帖子" : "回复了你的主题";
    item.title = `${name}${action}`;
    item.interaction = {
      topicId:row.topic_id,postNumber:row.post_number,commentId:row.comment_id,topicTitle:row.topic_title,
      excerpt:row.excerpt,actorName:name,action,
      actorHref:row.actor_status === "active" ? `/users/${row.actor_id}` : null,
      actorAvatar:row.actor_status === "active" ? row.actor_avatar : null,
    };
  }
}

export async function inboxTargetLocation(item: InboxItem) {
  if (!item.interaction) return null;
  const target = item.interaction;
  try {
    return await forumLocation(getForumRuntime(),target.topicId,
      target.commentId ? {commentId:target.commentId} : {postNumber:target.postNumber});
  } catch (error) {
