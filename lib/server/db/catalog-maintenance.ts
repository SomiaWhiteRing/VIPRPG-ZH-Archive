import { hasPermission, canMergeWorks } from "@/lib/authz/permissions";
import { getD1 } from "@/lib/server/db/d1";
import { findUserByEmail, type ArchiveUser } from "@/lib/server/db/users";
...
export async function mergeCreators(
  ...
  await db.batch([
    ...
    db
      .prepare(
        `UPDATE creator_aliases SET creator_id=? WHERE creator_id=?`,
      )
      .bind(target, source),
    db
      .prepare(
