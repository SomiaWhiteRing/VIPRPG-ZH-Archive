import type { CharacterAliasSuggestion, CharacterRoleKey } from "@/lib/character-names";
import { characterNodePath, type CharacterCategory } from "@/lib/character-index";
...
export type CharacterWork = CharacterWorkCredit & { credits: CharacterWorkCredit[] };
...
export async function getPublicCharacterDetail(id: number) {
  const db = getD1();
  const row = await db.prepare(`SELECT ch.id,ch.primary_name AS primaryName,
    ...
    WHERE ch.id=? LIMIT 1`).bind(id).first<DetailRow>();
  if (!row) return null;

  const [aliases, sources, categories, memberships, works, faces, materials] = await db.batch([
    db.prepare("SELECT name,language FROM character_aliases WHERE character_id=? ORDER BY language,name").bind(id),
    db.prepare("SELECT url FROM character_sources WHERE character_id=? ORDER BY sort_order,url").bind(id),
    db.prepare("SELECT id,parent_id AS parentId,label FROM character_categories"),
    db.prepare("SELECT category_id AS categoryId FROM character_category_memberships WHERE character_id=? ORDER BY category_id").bind(id),
    db.prepare(`SELECT w.id,wc.id AS creditId,COALESCE(w.chinese_title,w.original_title) AS title,w.original_title AS originalTitle,
      ...
      ORDER BY COALESCE(w.original_release_date,w.published_at,w.created_at) DESC,w.original_title,w.id,wc.sort_order,wc.id`).bind(id),
    db.prepare(`SELECT 'faceset:' || fs.id AS id,'faceset' AS kind,fs.blob_sha256 AS blobSha256,fs.width_px AS width,fs.height_px AS height
      ...
      ORDER BY binding.sort_order IS NULL,binding.sort_order,fs.source_order IS NULL,fs.source_order,fs.id`).bind(id),
    db.prepare(`SELECT 'material:' || m.id AS id,m.kind,m.blob_sha256 AS blobSha256,m.width_px AS width,m.height_px AS height
      ...
      ORDER BY m.kind,binding.sort_order IS NULL,binding.sort_order,m.id`).bind(id),
  ]);
  ...
  return {
    ...
    sourceUrls: (sources.results as { url: string }[]).map((source) => source.url),
    categories: (memberships.results as { categoryId: string }[]).map((membership) => ({
      id: membership.categoryId,
      path: characterNodePath(categories.results as CharacterCategory[], membership.categoryId),
    })),
