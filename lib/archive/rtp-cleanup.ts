 ...
 * The caller defines the candidate scope. Production uses exact RTP only.
 */
export class LcfReferenceScan {
  private readonly candidates: RtpFile[];
  private readonly names = new Map<string, RtpFile[]>();
  ...
  constructor(private readonly files: readonly RtpFile[], isCandidate: (file: RtpFile) => boolean) {
...
/** Production policy remains restricted to exact, known RTP identities. */
